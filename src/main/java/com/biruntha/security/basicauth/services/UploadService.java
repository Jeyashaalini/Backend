...
import com.biruntha.security.basicauth.repository.UploadRepository;
...
//			Integer id = uploadRepository.getMaxUploadId() + 1;
	...
	public ResponseEntity<Upload> getUploadById(String id) {
		try {
	...
	public ResponseEntity<Upload> deleteUpload(String id) {
		try {
	...
	public ResponseEntity<Upload> updateUpload(String id, Upload upload) {
		try {
			Optional<Upload> uploadData = uploadRepository.findById(id);
			if(uploadData.isPresent()) {
